...
<div class="flex min-h-screen bg-[#161616] text-white">
    ...
    <main class="flex-1 p-10 ml-64">
        ...
        <form action="{{ route('categories.store') }}" method="POST" class="bg-[#1e1e1e] border border-gray-700 rounded-2xl p-6 shadow-md shadow-black/20">
            @csrf
            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-300">Category Name</label>
                <input type="text" name="name" id="name" required class="mt-1 block w-full bg-[#161616] border border-gray-700 rounded-lg shadow-sm focus:ring-yellow-500 focus:border-yellow-500 text-gray-300 placeholder-gray-500" placeholder="Enter category name">
            </div>
